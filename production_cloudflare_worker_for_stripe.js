/**
 * Amy Wireless Care+ — Production Cloudflare Worker for Stripe Gateway
 * 
 * Features:
 * - POST /create (or /) -> Creates Stripe PaymentIntent with amount in cents
 * - GET  /check?id=pi_... -> Polls Stripe for live PaymentIntent status
 * - GET  /health -> Instant diagnostic testing endpoint for frontend & curl
 * - Full CORS support for GET, POST, OPTIONS
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    // 2. Health & Diagnostic Check (GET /health or GET /ping)
    if (path === "/health" || path === "/ping") {
      ...
      // Verify Stripe Secret Key validity by pinging Stripe balance endpoint
      try {
    ...
    // Guard: Ensure Stripe Secret Key is present for all operations
    if (!env.STRIPE_SECRET_KEY) {
      return new Response(JSON.stringify({
    ...
    // 3. Status Polling Endpoint (GET /check?id=pi_...)
    if (path === "/check") {
    ...
    // 4. Payment Intent Creation Endpoint (POST /create or POST /)
